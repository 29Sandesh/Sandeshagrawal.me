  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      constructor() {
        ...
        this.size = Math.floor(Math.random() * 4) + 2; // Pixel size
      ...
      particles.forEach((particle, index) => {
        ...
        for (let i = index; i < particles.length; i++) {
          ...
          if (distance < connectionDistance) {
            ctx.beginPath();
            ctx.strokeStyle = `rgba(0, 255, 0, ${0.15 - (distance / connectionDistance) * 0.15})`;
            ctx.lineWidth = 2;
            ctx.moveTo(Math.floor(particle.x), Math.floor(particle.y));
            ctx.lineTo(Math.floor(particles[i].x), Math.floor(particles[i].y));
            ctx.stroke();
          }
        }
      });
